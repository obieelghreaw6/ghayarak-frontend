...
export default function App() {
  ...
  async function handleModerateListing(id, decision, note) {
    try {
      ...
      // But an approval does need to reach the public feed — Home/Search
      // only ever fetched listings once on initial load, so without this
      // refetch the newly-approved listing stays invisible until the
      // next full page reload.
      if (decision === "approved") {
        try {
          const { listings: apiListings } = await listingsApi.list();
          setListings(apiListings.map(mapApiListing));
        } catch (e) { console.error("Could not refresh public listings", e); }
      }
...
--------------------------------------------------------------------- */
function AdminScreen({ listings, shops, revenue, orders, pendingListings, onModerate, onVerify, onRemove, onExit, onMarkCommissionSettled, onUpdateRefundStatus, onVerifyBankConfirmation }) {
  const { t, lang } = useLang();
  ...
  return (
    <div style={{ background: C.sandLight, minHeight: "100vh" }}>
      ...
      <div className="flex overflow-x-auto px-4 pt-3 gap-1 border-b" style={{ borderColor: C.line }}>
        {[{ id: "overview", label: t("tabOverview") }, { id: "moderation", label: t("tabModeration"), badge: pendingListings.length }, { id: "revenue", label: t("tabRevenue") }, { id: "listings", label: t("tabListings") }, { id: "shops", label: t("tabShops") }, { id: "settlements", label: t("settlementsTab") }, { id: "refunds", label: t("refundsAdminTab") }, { id: "bankTransfers", label: t("bankTransfersAdminTab") }].map((tb) => (
